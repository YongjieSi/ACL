...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        self.fc = Classifier(num_features = self.num_features, num_classes = self.args.num_all, temp = self.args.network.temperature)
        hdim=self.num_features
        self.slf_attn = MultiHeadAttention(1, hdim, hdim, hdim, dropout=0.5)
        self.set_fea_extractor_for_s2s()

    def forward_metric(self, x):
        feature_final, feature_inner = self.encode(x) #
        
        if 'cos' in self.mode:
            x = self.fc(feature_final)
           
        elif 'dot' in self.mode:
            x = self.fc(feature_final)
        
        return x, feature_final,feature_inner  
