...
import torch
from .sampler import  CategoriesSampler,  SupportsetSampler
...
def  get_testloader(args, session):
    ...
    if args.dataset in ['f2n', 'f2l', 'n2f', 'n2l', 'l2f', 'l2n', 'l2n2f','l2f2n','l2n2f2n','l2n2f2e']:
...
def get_old_testloader(args, session):
    ...
    # test on all encountered classes
    assert(session > 0)
    class_old = np.arange(0, num_base_class + (session - 1) * args.way)

    if args.dataset in ['f2n', 'f2l', 'n2f', 'n2l', 'l2f', 'l2n', 'l2n2f','l2f2n','l2n2f2n','l2n2f2e']:
        testset = args.Dataset.S2S(dataset=args.dataset, root=args.dataroot, phase="test",
                                index=class_old, k=None, args=args)
    elif 'nsynth' in args.dataset:
        testset = args.Dataset.NDS(root=args.dataroot, phase="test",
                                      index=class_old, k=None, args=args)
    elif 'librispeech' in args.dataset:
...
def get_base_dataloader_meta(args):
    ...
    elif 'FMC' in args.dataset:
        
        trainset = args.Dataset.FSDCLIPS(root=args.dataroot, phase='train', index=class_index, k=None)
        valset = args.Dataset.FSDCLIPS(root=args.dataroot, phase='val', index=class_index, k=100)
    elif 'nsynth' in args.dataset:
        trainset = args.Dataset.NDS(root=args.dataroot, phase='train', index=class_index, k=None, args=args)
    ...
    trainloader = torch.utils.data.DataLoader(dataset=trainset, batch_sampler=sampler, shuffle=False,num_workers=8,
                                                pin_memory=True)
...
def get_new_dataloader(args, session):
    ...
    elif args.dataset in ['f2n', 'f2l', 'n2f', 'n2l', 'l2f', 'l2n', 'l2n2f','l2f2n','l2n2f2n','l2n2f2e']:
        session_classes = np.arange(num_base_class + (session -1) * args.way, num_base_class + session * args.way)
    ...
    if 'nsynth' in args.dataset:
        testset = args.Dataset.NDS(root=args.dataroot, phase='test', index=class_new, k=None, args=args)
    if 'librispeech' in args.dataset:
        testset = args.Dataset.LBRS(root=args.dataroot, phase='test', index=class_new, k=None, args=args)
    if 'esc' in args.dataset:
        testset = args.Dataset.ESC(root=args.dataroot, phase='test', index=class_new, k=None, args=args)   
    if 'pqrs' in args.dataset:
        testset = args.Dataset.ESCS(root=args.dataroot, phase='test', index=class_new, k=None, args=args)  
        ...
        testset = args.Dataset.FSDCLIPS(root=args.dataroot, phase='test', index=class_new, k=100)
